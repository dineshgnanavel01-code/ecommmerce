    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-slate-950 px-4 py-5 text-slate-300 shadow-2xl transition-transform duration-300 lg:static lg:translate-x-0 lg:shadow-none ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="space-y-1">
          {items.map(({ label, icon: Icon }) => <button key={label} onClick={() => { setActive(label); setOpen(false); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${active === label ? "bg-violet-500 text-white shadow-lg shadow-violet-500/20" : "hover:bg-white/5 hover:text-white"}`}><Icon size={18} />{label}</button>)}
        </nav>
        <div className="mt-auto space-y-1 border-t border-white/10 pt-4">
          <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-white/5 hover:text-white"><Settings size={18} />Settings</button>
          <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-white/5 hover:text-white"><HelpCircle size={18} />Help Center</button>
          <div className="mt-4 rounded-2xl bg-white/5 p-4"><p className="text-xs font-semibold text-white">Need a hand?</p><p className="mt-1 text-[11px] leading-5 text-slate-500">Our support team is available 24/7.</p><button className="mt-3 text-xs font-semibold text-violet-300 hover:text-violet-200">Contact support →</button></div>
        </div>
      </aside>
    </>
  );
